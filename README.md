...
**按名称查找图层**：手点太慢时，点页面顶部的「按名称查找…」开弹窗，把**整个文档**（或已选中的组内）名称匹配的图层一次性选出来 ——
