...
`tests/panel-smoke.test.js` 用一套 DOM / Photoshop 桩把打包产物真跑一遍，验证初始化不抛错，并模拟两条完整链路：「点新建参考线版面 → PS 建出参考线 → 写记录 → 点应用重放」（断言下发给 PS 的**是菜单项而不是参数描述符**、三条记录来源各自能写进记录、「收藏当前版面」的两种存法、参数会损耗的版面在同尺寸下按坐标原样还原、原生 newGuideLayout 被接受却一条线都没画时退回插件自己建），以及「开查找弹窗 → 查找页搜索勾选 → 添加到查找列表 → 继续添加下一批 → 查找项页增删改 → 确认选中 → 改名」（断言列出的行与「找到 / 已勾选」统计、全选 / 反选、排序下拉、「含隐藏图层」默认勾选且不跨会话记忆、搜索按钮与回车不靠输入事件也能查、卡片的停用 / 折叠 / 删除 / 单项取消、预览跨卡片去重与「点行=从所有卡片取消」、编辑卡片时条件回填且保存是替换不是新增、清空全部后确认置灰且不下发、重开弹窗时查找项清零、下发给 PS 的 select 描述符、查找范围为「已选中的组内」时组内子层真的进得了预览、弹窗期间页面输入框被藏起、取消不留痕、背景层标注、加入后图层被删时跳过），还有「参考线分割」整条流水线（把工作文档 / 拍平层 / 读像素 / 粘出来的层都做成桩，断言**建选区用的是每格内容的紧贴外框而不是格子本身**、空白格不切且编号连续、落位跑偏时按预期外框补一条 move 挪回去而差 <1px 时不动、收尾统一编组、底稿层取的是「合并后唯一可见的非组图层」而不是栈顶、imaging 读不动时先栅格化再重读，以及切不了的情况一律在下发任何描述符之前就拦住），「智能分割」的选区（造两个外框交叠的图形，先断言按整框会漏进对方的像素，再**逐像素**核对实际下发的选区一个不漏一个不多，以及 PS 不认 `addTo` 时退回整框而不是整批失败），「批量改尺寸」的四步向导与整批执行（校验拦得住、下发给 PS 的 imageSize / canvasSize 数值与锚点枚举、生成的文件名与 JPG 质量换算、多尺寸时一个文件只打开一次靠快照回退、小图默认放大到目标、跑完主按钮变「返回」而回上一步改了参数就变回「开始批量修改」、「打开输出文件夹」走 PS 的脚本桥（临时 `.jsx` 的内容与 `AdobeScriptAutomation Scripts` 描述符都断言到）而桥不通才退 `openPath`、两条都不通就把每条理由摊在状态栏上并把路径复制进剪贴板、预设与「自定义」来回切时参数和参数区一起还原、四步之间随便切换不设卡而「开始批量修改」会跳回缺东西的那一步并报出第几步、位置留空（存回原位置）而反查父目录失败时几种 file: 写法都试过且每一次的原话都摊出来、点文件夹图标后图标换成文件夹名而「改回原文件夹」退回默认、已选清单把每一张都列出来、命名示例报到底部状态栏且两边都压成「头…尾」），以及「完整切图」的命名与画布（**落盘文件名保留图层名里的下划线与大小写**、不拼 PSD 名、组名逐层进名、项目名只多一段前缀、**「导出选中」时命名从选中项算起**（只点组里的一层就只有层名、点组则组名在）、Symbols 的组名同样原样进名、路径为空时收口成 symbol、中文转拼音、只差大小写也算重名、开着「完整导出超出画布部分」时扩画布必须排在 `mergeVisible` 之前、且只扩到这一轮参与合并的层的范围而不是整个 PSD，内容没超出画布时连 `crop` 都不发）。

## 输入框通用行为

面板里**每一个文字输入框**都是同一套（`panel.js` 的 `bindTextField` / `TEXT_FIELDS`）：

- **点进去**：外框描边转亮青（高亮当前正在编辑的那个），框内自动清空，直接敲新值不用先删旧的；原值挪到灰字提示里，全程看得见。
- **移开**：高亮还原。框里输了新值就按新值；**什么都没输就把原值放回去**。
- 清空只改界面、不当成「用户把值删空了」上报，所以正在编辑时直接点按钮也不会读成空 —— 取值一律走 `fieldValue()`。

> 滚动条一律不画（滚动照常）。UXP 的滚动条由原生层绘制、压在所有 DOM 之上，`z-index` 管不着，弹窗和按钮圆角都会被它横穿。
...
- **文字编辑控件恒绘制在所有 DOM 之上，z-index 无效** —— 这是 [Adobe 官方已知问题](https://developer.adobe.com/photoshop/uxp/2022/uxp-api/known-issues)，换成普通 `<input>` 也一样。悬停说明浮层压到 `sp-textfield` 时会被顶穿，解法是浮层出现期间把输入框 `visibility: hidden`（必须设在控件**自身**，设在父级不生效）。**自绘下拉的菜单同样是「浮层」**：向下展开时会被下面的输入框切掉一半，所以菜单开合也要走同一套遮挡逻辑（`setDdMaskedFields`）。
...
- **`mergeVisible` 之后，`doc.layers[0]` 不是拍平出来的那一层**。Merge Visible 只合并**可见**图层，隐藏的原样留在图层栈里 —— 栈顶很可能是某个隐藏的文字层 / 调整层 / 智能对象，拿它的 id 去 `imaging.getPixels` 就报 `Unsupported layer type`（只有目标本来就在最顶上时才恰好不出错）。判据要换成「合并后唯一可见的**非组**图层」（组自己的 `visible` 也是 true，必须排掉）。另外 `imaging.getPixels` 只认像素图层，读不动就先 `rasterizeLayer` 再读一次 —— 在临时工作文档里栅格化没有副作用。
- **UXP 打不开文件夹，得借 Photoshop 的脚本引擎**。两条 UXP 路都是死的，真机各撞过一次：`shell.openPath` 拿路径的**扩展名**去比 manifest 里 `launchProcess.extensions` 的白名单，文件夹的扩展名是空的，哪怕白名单里写了 `""` 照样拒（`Extension "" is not accepted for the path …`）；`shell.openExternal` 则**不收 `file:` 协议**（`URI scheme "file" is not accepted`），官方文档写明「file scheme is not allowed for openExternal，请改用 openPath」—— 两句话合起来就是「文件夹没法开」，`launchProcess.schemes` 里放 `"file"` 也没用。
  走得通的是 PS 自带的 **ExtendScript 引擎**：把一段 `new Folder(p).execute()` 写成临时 `.jsx`，用 batchPlay 播 `AdobeScriptAutomation Scripts`（`javaScriptName` 给脚本的本机路径）—— 这条路不经过 UXP 的 `launchProcess` 关卡。源码由 `buildRevealJsx()` 生成（路径按 `JSON.stringify` 嵌进去，反斜杠和引号都不会拼坏），Windows 上 `execute()` 万一返回 false 再兜一层 `app.system('explorer …')`。`openPath` 留作第二条路（Mac 上文档说文件夹可以）。
  另外 `openPath` / `openExternal` **打不开时不抛异常**，而是 resolve 出一句错误描述（成功是空串）—— 只包 `try/catch` 等于永远发现不了失败。
- **manifest 里的权限是装载插件时读的**。改了 `requiredPermissions`（`localFileSystem`、`launchProcess`）必须在 UDT 里 Remove → Add 再重启 Photoshop，热重载 JS 不会让新权限生效 —— 表现是代码明明是新的、却报没权限那一类的错。
- **选中一个组时，`doc.activeLayers` 连它的所有子层一起给**，「只选了组」和「组与组内层都点亮了」在 DOM 那头完全一样。要区分得读文档描述符上的 **`targetLayersIDs`** —— 它只报用户真正点亮的项：真机实测（PS 27.7.0）只选组 G 报 **1 项**、「G + G 内部的层」报 **2 项**、「G + 嵌套组里的孙子层」也是 2 项，从不把没点亮的子层算进来。重命名走的就是这条（`trueSelectedLayers`）。
  - 列表项是**图层引用**不是整数（ExtendScript 用 `getReference(i).getIdentifier(layerID)`，UXP 里序列化成 `{_ref:'layer',_id:N}`）。
  - **别用 `targetLayers` / `targetLayersIndexes`**：那两个给的是 index，要额外判断文档有没有背景层做偏移。
  - 把组当整体处理的功能（切图 / 排版 / 平移 / 建组）仍走 `activeLayers` + 剔除后代那条路（`selectedLayers`），它们本来就要「组算一次」。
- **单步撤销优先官方 `doc.suspendHistory`**。`smart-object.js` 里那套「快照折叠」只在转智能对象流程真机验证过，别往别处复用 —— 用在改名流程上出现过历史倒回文档打开状态、改动全丢。

## 手动验证清单（在 Photoshop 里）

- [ ] `layer-tree`：红色图层 `label==='red'`、蓝色组 / 蓝色图层 `label==='blue'`、结构与图层面板一致
- [ ] `exporter`：普通层紧贴像素、超画布开关两态、蓝组合并排除红色（含嵌套红色**组**）、全透明跳过
- [ ] 蓝色图层合并：组里三层标蓝 → **只出一张**，名字是最下面那层；中间隔一层不标蓝 → 蓝的仍合成一张、被隔开那层自己一张；开「只对选中切图」只点中其中一层 → 那一张仍是三层合并的
- [ ] 端到端切图：命名 / 去重 / 红蓝规则 / 隐藏开关 / 同名询问 / ESC 暂停与终止恢复
- [ ] 导出选中的命名起点：开「只对选中的图层 / 组切图」→ **只点亮组里的一层**（别点组）导出，文件名只有这一层的名字，不带组名；填上项目名则是 `项目_层名`；改成**点亮组本身**再导，组名回到名字里
- [ ] 切图命名：图层 `WG_effect_8` 导出成 `WG_effect_8.png`（**不带 PSD 名、下划线与大小写都在**），中文层名仍是拼音首字母
- [ ] 完整导出超出画布部分：把一个图层拖到一半在画布外 → **开**时导出的图含画布外那半边，**关**时只有画布内那半边；**再拿一个「有图层远远飘在画布外」的大 PSD 整批切一遍，速度要和关掉开关时相当**（慢十倍就说明又在按全 PSD 扩画布了）
- [ ] Symbols 切图：图标未超定位格按定位格尺寸导出，超出则补白居中
- [ ] 批量重命名：四模式预览与写回、n 编号（起始 / 位数 / 方向按面板序）、Ctrl+Z 一步撤销；**只选中几个组时预览里只有组名、组内的层不出现也不被改**；再按住 Ctrl 点亮组内的某一层 → 预览里组名和那一层都在，各改自己的；**预览行首的图标要跟类型对上**（组=文件夹、层=图片框）；选够 300 个以上 → 预览只画 300 行且末尾有说明，但应用后全部被改
- [ ] 智能分割：开关两态、网格素材按格拆、原图层未改动；**两个外框交叠的图形（一个的手臂伸到另一个上方 / `C` 形圈住一个圆点）→ 切出来的每一层都只有自己的像素，看不到邻居的碎片**
- [ ] 参考线分割：**每块都落在原来的位置上（不跑位）**、空白格跳过且编号连续、切组 / 文字层也能切、合并可见内容不看选中、只拖竖线切成整条列、无参考线时按钮置灰、50+ 块先弹确认且取消后零改动、结果进新组、原对象未改动
- [ ] 批量改尺寸：混放横竖图的文件夹 + 固定宽高等比适应 → 输出都是目标尺寸、图居中、空白透明；换等比填充 + 锚点「上」→ 从下方裁；最长边 2048 横竖各对；512 的小图配 1024 目标 → 确实放大到 1024；多尺寸 1024/512/256 → 一个文件出三张且**只打开一次**（看速度）；保持结构 + 第二遍不重复处理输出目录里的图；**位置留空**（默认，存回原文件所在位置）多选图片也能落到原目录（报「定位不到原文件所在目录」时把括号里那一串原话整段记下来 —— 每种 file: 写法各一条，那是系统给的）；点文件夹图标指定位置 → 图标换成文件夹名，点「改回原文件夹」退回默认；跑完点「打开输出文件夹」真的开得起来（开不起来时状态栏要列出每条拒绝理由，且路径已进剪贴板）；顶上 1234 前后随便点都换页、点「开始批量修改」才被拦并报出「第几步 + 缺什么」；预设选「缩放 50%」再选回「自定义」→ 参数和参数区都回到之前那套；「输出设置」后面的问号悬停出说明；命名示例出现在底部状态栏；同名三档各试；**来源选当前文档跑完，原文档一点没变**；处理中停止 → 已存的在、没有多余窗口；放一个损坏文件 → 整批不中断且失败记录有原因；参数与预设跨会话记得住
- [ ] 批量转智能对象 / 新建独立组：逐个独立不合并、层级与属性不变、一步撤销
- [ ] 一键排版：行列识别正确、边缘间距精确、锚点未动、四方向扩画布、锁定层中止并提示
- [ ] 快速平移：八个方向各走对、斜向两个值、正向不夹带另一轴、负数翻向、父子去重不双倍、锁定层跳过、Enter 与方向键、一步撤销
- [ ] 快速平移的复制移动：原对象不动、副本吃位移且留在选中态、整次仍是一步撤销
