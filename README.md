...
面板顶部七个磁贴（排成两行）各对应一个功能页，启动默认停在「重命名」。
...
> **打开时的初值**：和你手动点菜单打开时**一模一样** —— 首次是 Photoshop 的出厂默认，之后每次都自动带出**上一次确定的设置**。这份记忆归 Photoshop 管，插件不插手。想用更早建过的某一版，去下面的「最近使用 / 收藏版面」点「应用」。
>
> 为什么必须走菜单项：只要给 batchPlay 传了 `newGuideLayout` 描述符（哪怕是空的），Photoshop 就按那份描述符初始化弹窗、缺的键用出厂默认（装订线 20 像素 / 数量 2 / 边距 20 像素）补上，它自己记的「上次设置」反而被顶掉 —— 表现就是每次打开都像第一次。所以弹窗改成播放菜单项 `select menuItemClass newGuideLayout` 来开。只有菜单项这条路走不通时，才退回「插件按上一条记录预填描述符」，状态栏会注明。

### 版面记录

弹窗点了确定之后，插件会把这一版写进「最近使用」。记录里存的是**你真正建出来的那一版**，不是面板里的猜测。有三条独立来源，谁先到算谁的（按参数去重，不会记成两条）：

1. `newGuideLayout` **动作通知** —— PS 发这个事件时附带实际执行的参数，连你自己走「视图 > 新建参考线版面」建的也能记下。但**不是每个版本都发**（真机上就没见它发过），所以只当锦上添花，不作依赖；
2. batchPlay 的**返回值** —— 只有走「预填描述符」那条兜底路才有（真机形状：参数平铺在顶层，列的键是 `colCount` / `colWidth` / `colGutter`，**不是**与行对称的 `columnCount`；末尾还跟着参考线颜色 `$GdC*`）；
3. **前后对比反推**（实际在用的就是这条）—— 上面两条都没给到参数时，插件对比弹窗前后文档里的参考线，从**多出来的那批线**把列数 / 行数 / 装订线 / 边距 / 是否清除现有算回来：交替间距认列宽与装订线，等间距认装订线为 0 的等分，单独的首尾两条先当边距线剥掉。纯几何、有单测；认不出来的形状宁可不记，也不记错。

> 没走插件按钮、自己去菜单里建的那一版，插件不一定收得到通知（也就不会自动进记录）。想留下它，用下面的 **收藏当前版面**。

记录里的摘要**列 / 行 / 边距三块都会写出来**，没设的那块明写「未设置」（例如 `列 未设置；行 5 · 高度自动 · 装订线 0px；边距 未设置`）—— 一眼分得清「那次确实没设列」和「列没被读到」。

> 「列」要生效，弹窗里**「数字」必须填**：只勾上「列」、只填装订线而数字留空，Photoshop 建出来的就是 0 列，记录里自然是「列 未设置」。行同理。

创建成功后状态栏只报一句摘要（如 `已创建参考线版面：6列 / 8行`），完整参数在「最近使用」里看。三条来源都没结果时，状态栏会**如实说明本次没有记录**并附上 Photoshop 回传的参数键名；参数读到了但其中有插件不认识的键，也会在末尾提示 `有没认出来的参数：xxx` —— 键名在不同版本里若有差异，这行就是定位线索（插件对每个参数都备了若干别名，例如列数会认 `columnCount` / `numberOfColumns` / `columnsCount`）。
...
| 收藏版面 | **应用** / **改名** / **删除**；来源既可以是最近使用里的 ☆，也可以是「收藏当前版面」 |

**应用**不弹窗，直接重放那一版（走 Photoshop 自己的版面命令，万一不可用则由插件直接创建，结果一致；原样快照则按存下来的坐标逐条建）。收藏可自定义名称（最长 24 字），不受 20 条上限影响。删除记录**只删插件里的数据，不会动文档里的参考线**。
...
`tests/panel-smoke.test.js` 用一套 DOM / Photoshop 桩把打包产物真跑一遍，验证初始化不抛错，并模拟「点新建参考线版面 → PS 建出参考线 → 写记录 → 点应用重放」这条链路，断言下发给 PS 的**是菜单项而不是参数描述符**、三条记录来源各自能写进记录、以及「收藏当前版面」的两种存法。
