...
面板左侧竖排九个功能入口（图标 + 名称），各对应一个功能页，启动默认停在「重命名」；设置钮在功能栏底部，点它打开[设置（关于 / 检查更新）](#十一检查更新)。顶栏（品牌 + 版本号）与底部状态栏固定不动，只有右侧内容列滚动。
...
> **输入框的取值模型**：当前值以灰字提示显示，点进去框自动清空可直接输入，按 **Enter** 确认并退出输入框；点进去没改就移开，仍保持进来时的数值（这一套现在是**全插件通用**的）。整页设置跨会话记忆。
...
| 记忆多久 | 项 |
|---|---|
| **跨会话**（localStorage） | 一键排版全部设置；快速平移的方向；快速绘制表格全部设置；参考线助手的最近使用 20 条 + 收藏版面；「自动检查更新」开关与上次检查时间 |
